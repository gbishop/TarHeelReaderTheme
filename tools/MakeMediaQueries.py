...
print(
    "@media only screen and (min-aspect-ratio: 4/3) and (min-height: %dpx) { body { font-size: %.2fpx; } }"
    % (sizes[-1], sizes[-1] / 36)
)
print(
    "@media only screen and (max-aspect-ratio: 4/3) and (min-width: %dpx) { body { font-size: %.2fpx; } }"
    % (sizes[-1], sizes[-1] / 48)
)
