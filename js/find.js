...
define([
    ...
    function findRender(url, query) {
        ...
        store.find(url).then(function(data) {
            ...
            $newPage
                .empty()
                .append(
                    templates.render("heading", {
                        settings: true,
                        chooseFavorites: true
                    })
                )
                .append(
    ...
    function moveSelection(direction) {
        ...
        var i = 0;
        var toSelect =
            direction > 0 ? $(targets[0]) : $(targets[targets.length - 1]);
        ...
        toSelect.scrollIntoView({
            ...
            complete: function() {
                ...
                var top = wt + (wh - b) / 2;
                toSelect
                    .clone() // make a copy of the selected book
                    .addClass("preview")
                    ...
                    .animate(
                        {
                            // animate the book zooming up to final size
                            top: top,
                            left: left,
                            fontSize: fs
                        },
                        300
                    ) // not too slow or it jiggles a lot
    ...
    $.subscribe("/find/select", function(e, name, code) {
        ...
        var link;
        if (selected.length > 0) {
            if (selected.is("a")) link = selected;
            else link = selected.find("a");
    ...
    function setFindKeyMap(page) {
        keys.setMap(page, {
            "left down enter c pageup": "/find/select",
            "right space m pagedown": "/find/next",
            up: "/find/prev"
        });
    }
    ...
    $(document).on(
        "click",
        ".chooseFavorites img.favoriteNo, .chooseFavorites img.favoriteYes",
        function(ev) {
    ...
    $(document).on(
        "click",
        ".favorites-page.chooseFavorites .thr-favorites-icon",
        function(ev) {
