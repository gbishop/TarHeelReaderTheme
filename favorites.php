...
function new_favorites_url($q)
{
    ...
    foreach (
        array('bgcolor' => 'pageColor', 'fgcolor' => 'textColor')
        as $old => $new
    ) {
...
if (array_key_exists('books', $_GET)) {
    // old format URL, convert it before redirect
...
if (
    !array_key_exists('favorites', $_GET) &&
...
if ($collection) {
    $favorites = $wpdb->get_var(
        $wpdb->prepare(
            "SELECT booklist FROM $collections_table WHERE slug = %s",
            $collection
        )
    );
...
thr_header('favorites-page', array(
    'settings' => true,
